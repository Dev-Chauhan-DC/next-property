...
const UpdateAddressModal: React.FC<Props> = ({ deleted, onOutsideClick, show, updated, agentAddress }) => {
    ...
    const agentAddressUpdateHandle = async (id: number, data: IAgentAddress) => {
        try {
            await agentAddressUpdate(id, data)
    ...
    const agentAddressDeleteHandle = async () => {
        try {
            if (!agentAddress?.id) return
            setDeleteLoading(true)
            await agentAddressDelete(agentAddress?.id)
        } catch (error) {
            console.error(error)
            Toast.show(getError(error))
        } finally {
    ...
    useEffect(() => {
        if (agentAddress?.title) {
            setFormData(e => ({ ...e, title: agentAddress.title }))
        }
        if (agentAddress?.address) {
            setFormData(e => ({ ...e, address: agentAddress.address }))
        }
        if (agentAddress?.latitude) {
            setFormData(e => ({ ...e, latitude: agentAddress.latitude }))
        }
        if (agentAddress?.longitude) {
            setFormData(e => ({ ...e, longitude: agentAddress.longitude }))
        }
    }, [agentAddress])
    ...
    return (
        <Dialog show={show}>
                ...
                <View className='flex-1 overflow-auto flex flex-col gap-4 items-center w-full'>
                    ...
                    <Text
                        onPress={async () => {
                            ...
                            await agentAddressDeleteHandle()
                            deleted && deleted()
            ...
            <DialogFooter
                onPress={async () => {
                    if (!agentAddress?.id) return
                    await agentAddressUpdateHandle(agentAddress.id, formData)
                }}
            />
        </Dialog>
    )
