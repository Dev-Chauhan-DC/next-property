...
import { builderAddressCreate } from '@/src/data/network/services/builderAddress';
...
const CreateAddress: React.FC<Props> = ({ onOutsideClick, show, updated }) => {
    ...
    const builderAddressCreateHandle = async (data: Partial<IBuilderAddress>) => {
        try {
            setLoading(true)
            const result = await builderAddressCreate(data);
            updated && updated()
        } catch (error) {
            console.error(error)
        ...
        <Dialog>
            ...
            <DialogFooter
                onPress={async () => {
                    ...
                    await builderAddressCreateHandle(formData)
